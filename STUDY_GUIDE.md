...
- **What:** A LAN host/join flow where one player clicks "Create Server" (desktop/native only) and shares their local IP, while joining players type that IP and connect on the selected port. **HTML5 / Web export cannot host:** the server uses `TCPServer.listen()`, which browsers do not allow, so the Web build hides "Create Server" and only shows join. The host must run a desktop export (e.g. macOS). Mobile browsers join with `ws://<host-LAN-IP>:9080` the same as the desktop client.
- **How:** `game.gd` owns a small multiplayer control panel (Create Server, Join Server, IP input, port input, status label). On Web, the status label hints that the desktop app hosts, and `IpAddressInput` uses `virtual_keyboard_enabled` so tapping the field can open the on-screen keyboard; the Web preset enables `html/experimental_virtual_keyboard` for better browser integration. Host path instantiates `Server.tscn`, sets the port, detects a private local IPv4 address, and shows `Hosting on: <ip>:<port>`. Join path validates the typed IPv4, instantiates `Client.tscn`, and passes host + port into `configure_connection(...)`. On `_ready()`, each side picks a random **callsign** from `Globals` name pairs and logs it.
- **Web touch play:** After join, `TouchControlsLayer` (`ui/touch_controls/`) is shown only when `OS.has_feature("web")`. It sets `Globals.touch_steer_axis` and `Globals.touch_fire_held`; the local `player_space_ship.gd` merges those with `ui_left` / `ui_right` / `ui_accept` so keyboard and on-screen controls work together.
...
- **Web cannot host:** The current server is TCP listen + WebSocket accept; only native/desktop builds can create the lobby. Web players must join a desktop host (same LAN and firewall rules apply).
- **HTTPS vs `ws://`:** If the game page is served over HTTPS, browsers may block non-TLS `ws://` to the host (mixed content). Production Web may need `wss://` and a TLS-terminated proxy.
